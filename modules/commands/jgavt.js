...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    switch (handleReply.type) {
      ...
      case "create": {
       ...
       return request(encodeURI(`https://www.taoanhdep.tk/taoanhdep/avatarwibu?id=${id}&chu_nen=${nen}&chu_ky=${ky}&color=${color}`)).pipe(fs.createWriteStream(__dirname+'/cache/tad.png')).on('close',() => callback());    
    }
   }
 }
...
return api.sendMessage(`===> Nhân Vật: ${text} \n===>  ID: ${idz}\n===> Màu Mặc Định: ${color}`, event.threadID, event.messageID)
    ...
    const name = this.config.name;
    var id = args[0];
    axios.get(`https://www.taoanhdep.tk/taoanhdep/list`).then (res => {
